...
//                kh.setDiemthuong(rs.getInt(5));
    ...
    @Override
    public List<GioHangDTO> getListHoaDonChiTietByMaHd(String MaHD) {
        List<GioHangDTO> getList = new ArrayList<>();
        try {
            String sql = "SELECT ctsp.Ma, ctsp.Ten, hdct.Soluong, hdct.Dongia, ms.Ten, cl.Ten, kc.Ten, km.HinhthucKM, km.Giatrigiam FROM HoaDonChiTiet hdct JOIN HoaDon hd ON hd.Id = hdct.IdHD\n"
                    ...
                    + "								 JOIN KhuyenMai km ON ctsp.IdKM = km.Id\n"
                    + "		WHERE hd.Ma = ?;";
            Connection conn = DBConnection.openDbConnection();
            PreparedStatement pr = conn.prepareStatement(sql);
            pr.setString(1, MaHD);
            ...
            while (rs.next()) {
                ...
                ghdto.setHinhThucGiamGia(rs.getString(8));
                ghdto.setGiamGia(rs.getDouble(9));
                getList.add(ghdto);
            }
        } catch (Exception e) {
            e.printStackTrace();
