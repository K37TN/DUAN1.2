...
public class ChiTietSPRepository implements IChiTietSPRepository {
    ...
    final String SQL_SELECT_ALL = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,SoLuongTon,GiaNhap,GiaBan,MoTa FROM dbo.ChitietSP WHERE SoLuongTon > 0";
    final String SQL_SELECT_BY_MA = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,SoLuongTon,GiaNhap,GiaBan,MoTa FROM dbo.ChitietSP WHERE Ma = ?";
    final String SQL_SELECT_BY_SL = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,SoLuongTon,GiaNhap,GiaBan,MoTa FROM dbo.ChitietSP WHERE SoLuongTon = 0";
    final String SQL_SELECT_BY_TEN = "SELECT Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,SoLuongTon,GiaNhap,GiaBan,MoTa FROM dbo.ChitietSP WHERE SoLuongTon > 0 AND ten LIKE ?";
    final String SQL_INSERT = "INSERT INTO dbo.ChitietSP\n"
            + "( Ma, Ten, IdNsx, IdMauSac, IdDMuc, IdKC, IdKM, IdCL, IdTH, MoTa, SoLuongTon, GiaNhap, GiaBan)\n"
            + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
//            Ma,Ten,IdNsx,IdMauSac,IdDMuc,IdKC,IdCL,IdTH,IdKM,SoLuongTon,GiaNhap,GiaBan,MoTa,QrCode
    ...
    public int insert(ChiTietSP x) {
        return DBConnection.ExcuteQuery(SQL_INSERT, x.getMa(), x.getTen(), x.getIdnsx(), x.getIdmausac(), x.getIddanhmuc(), x.getIdkc(), 1, x.getIdcl(), x.getIdth(), x.getMota(), x.getSoluongton(), x.getGianhap(), x.getGiaban());
    }
